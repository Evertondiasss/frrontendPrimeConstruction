// funcionarios.js — protegido com authFetch
...
const API_VINCULO    = 'https://backendprimeconstruction-production.up.railway.app/api/funcionario_obras';

// ==================================================================
// WRAPPER DE FETCH COM TOKEN + TRATAMENTO DE 401
// ==================================================================
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const getInitials = nome =>
    nome.trim().split(/\s+/).map(p=>p[0]).slice(0,2).join('').toUpperCase() || 'F';
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      // 1 — Criar funcionário
      const res = await authFetch(API_BASE_FUNC, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...getAuthHeaders() },
        body: JSON.stringify({
          nome, cpf, telefone, data_admissao, cargo, salario, id_obra
        })
      });

      const novoFuncionario = await res.json().catch(()=>({}));

      if (!res.ok)
        throw new Error(novoFuncionario?.error || "Erro ao cadastrar funcionário.");

      // 2 — Criar vínculo com obra
      const vinculo = {
      ...
      try {
        const vinculoRes = await authFetch(API_VINCULO, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', ...getAuthHeaders() },
        ...
        if (!vinculoRes.ok)
          throw new Error('Erro ao vincular funcionário à obra.');
